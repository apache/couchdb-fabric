...
update_mrview(DbName, {DDocId, Rev}, ViewName, Args0) ->
    ...
    couch_util:with_db(DbName, fun(Db) ->
        ...
        {ok, Pid, _} = couch_mrview:get_view_index_pid(
            Db, DDoc, ViewName, convert_record(Args0)),
...
map_view(DbName, DDoc, ViewName, Args0, DbOptions) ->
    set_io_priority(DbName, DbOptions),
    Args = fix_skip_and_limit(convert_record(Args0)),
...
convert_record(Record) when element(1, Record) =:= mrargs ->
    couch_mrview_util:convert_record(Record);
